...
/// <summary>
///     The <see cref="ReverseEnumerator{T}"/> is a basic implementation of the <see cref="IEnumerator{T}"/> interface for iterating backwards.
/// </summary>
/// <typeparam name="T"></typeparam>
public unsafe struct ReverseEnumerator<T> : IEnumerator<T> where T : unmanaged
{
    private readonly T* _list;
    private readonly int _count;
    private int _index;
    ...
    /// <param name="list">The <see cref="UnsafeList{T}"/>.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal ReverseEnumerator(T* list, int count)
    {
        _list = list;
        _count = count;
        _index = count+1;
